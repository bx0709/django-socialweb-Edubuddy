...
class MyProfileUpdateView(UpdateView):
    model = MyProfile
    fields = ["name" ,"dob", "college" ,"address","gender","phone_no","degree","passing_year" ,"interested" ,"hobbies" ,"language" ,"pic"]
...
class MyProfileListView(ListView):
    ...
    def get_queryset(self):
        ...
        if si == None:
            si = ""
        profileList =  MyProfile.objects.filter(Q(name__icontains = si) | Q(college__icontains = si) | Q(gender__icontains = si) | Q(degree__icontains = si)).order_by("-id");
        for prof in profileList:
            prof.followed = False
            ob = FollowUser.objects.filter(profile = prof, followed_by=self.request.user.myprofile)
            if ob:
                prof.followed = True
        return profileList
